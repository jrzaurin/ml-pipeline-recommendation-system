...
def _compute_recency_factor(df, xmid=365, tau=160, top=1):
    present = df.reviewDate.max()
    df["days_to_present"] = present - df["reviewDate"]
    df["days_to_present_inv"] = df.days_to_present.max() - df.days_to_present
...
def _sample_negative_test(group, all_items, n_pos, n_neg, rseed=1):
    np.random.seed = rseed
    rated_items = group.apply(list).reset_index().asin.tolist()
...
def time_train_test_split(df, suffix, test_fraction=0.2, n_years=5):
    ...
    train_df.to_feather(PROCESSED_DATA_DIR / "_".join(["train", suffix]) + ".f")
    valid_df.to_feather(PROCESSED_DATA_DIR / "_".join(["valid", suffix]) + ".f")
    test_df.to_feather(PROCESSED_DATA_DIR / "_".join(["test", suffix]) + ".f")


def leave_one_out_train_test_split(
    df, n_years=5, min_reviews_per_user=7, n_pos=4, n_neg=99
):
    ...
    ).reset_index(drop=True)[["reviewerID", "asin", "reviewDate", "overall"]]
    ...
    train = pd.merge(
        df_recent, test, on=["reviewerID", "asin"], how="outer", suffixes=("", "_y")
    )
    ...
    with Pool(cpu_count()) as p:
        non_rated_items = p.map(
            partial(_sample_negative_test, all_items=all_items, n_pos=n_pos, n_neg=n_neg),
            [gr for _, gr in df_recent.groupby("reviewerID")][:10000],
        )
    end = time() - start
